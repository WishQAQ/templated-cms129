...
function AddFilter($channelid, $type=1, $fieldsnamef, $defaulttid, $loadtype='autofield')
{
	...
	$filterarr = wwwcms_filter(stripos($_SERVER['REQUEST_URI'], "plus/list.php?tid=") ? str_replace($nofilter, '', $_SERVER['REQUEST_URI']) : $GLOBALS['cfg_cmsurl']."/plus/list.php?tid=".$tid);
    ...
    if(is_array($dtp->CTags))
    {
        foreach($dtp->CTags as $tida=>$ctag)
        {
			...
			if(($loadtype!='autofield' || ($loadtype=='autofield' && $ctag->GetAtt('autofield')==1)) && in_array($ctag->GetName(), $fieldsname) )
            {
				...
				$dede_addonfields .= '<li><a class="bg167bbe" style="font-weight: bold;color:#fff;">'.$ctag->GetAtt('itemname').'</a><li>';
				switch ($type) {
					case 1:
						$dede_addonfields .= (preg_match("/&".$ctag->GetName()."=/is",$filterarr,$regm) ? '<a title="全部" href="'.str_replace("&".$ctag->GetName()."=".$fields_value,"",$filterarr).'">全部</a>' : '<li><a class="bg168bbe" style="color:#fff;width:70px;">全部</a></li>').'';
						...
						for ($i=0; $i<count($addonfields_items); $i++)
						{
							...
							$dede_addonfields .= ($fields_value!=urlencode($addonfields_items[$i]) ? '<li><a href="'.$href.'">'.$addonfields_items[$i].'</a></li>' : '<li><a class="bg168bbe" style="color:#fff;width:70px;">'.$addonfields_items[$i].'</a></li>')."";
						}
